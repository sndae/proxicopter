...
void CPersonView::ExecuteCntxMenuCmd(eMenuCmd eCmd)
{
	...
	else
	{
		...
		if(poPerson)
		{
			...
			if(!GetDocument()->SelectPhoneTypeWhereId(oSubscrPhoneNumb.m_nPhoneId, oPhoneType))
				return;
		...
		CPersonDlg oEditDlg(eCmd, oCitiesArr, oPhoneTyopesArr, oSubscriber, oSubscrPhoneNumb, oPhoneType, oCity);	
		...
		case eCmdFind:
			...
			RecreateColumnsContent(CPersonDoc::eColPhoneNumber, FALSE);
...
void CPersonView::RecreateColumnsContent(CPersonDoc::eColumn eColumn, const BOOL bResetFilter)
{
	m_PersonsArray.RemoveAndFreeAll();
	// запълване на листът с редове, спрямо последно наложеният филтър 
	if(!GetDocument()->SelectAll(m_PersonsArray, eColumn, TRUE, bResetFilter))
		return;
